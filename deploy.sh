...
if [ -r /dev/tty ]; then
	...
	./gradlew --no-daemon publishAllPublicationsToGitHubPackagesRepository -PskipReleaseConfirmation=true </dev/tty
	...
	./gradlew --no-daemon publishAllPublicationsToGitHubPackagesRepository -PskipReleaseConfirmation=true
